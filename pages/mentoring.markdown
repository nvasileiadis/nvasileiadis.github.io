---
layout: page
title: Mentoring — Nikos Vasileiadis | Engineering Leadership
nav_title: Mentoring
permalink: /mentoring/
description: "Free peer-learning mentoring for engineers stepping into management and engineering managers who want to grow — by Nikos Vasileiadis, Software Engineering Leader based in Leeds, UK. Two positions, opened deliberately."
---

I've been mentoring people since before it was part of any job title — from teaching adults their first computer skills in Greece, to developing engineers who've gone on to lead teams of their own. It's the part of leadership I'd do even if nobody was watching.

## Who this is for
{: .icon-compass}

I mentor people who are serious about the people side of engineering:

- **Engineers considering the move into management** — you're weighing the transition, or you've just made it and the ground feels unfamiliar.
- **Engineering Managers who want to grow** — you're in the role and want to sharpen your practice: team health, delivery, difficult conversations, organisational design.
- **Peers** — leaders at a similar stage to mine who want structured peer learning. Some of the best conversations I've had are with people who could just as easily be mentoring me.

If you're looking for interview prep, CV reviews, referral pipeline or to hire your engineers — unfortunately that's not what this is for.

## How it works
{: .icon-briefcase}

<div class="leadership-card" markdown="1">
**Free, always.** I do this to pass on what I've learned and to keep learning from other people's stories and experiences. No fees, no upsell, no coaching packages.
</div>

<div class="leadership-card" markdown="1">
**Your format.** Synchronous: a regular half hour call where we work through real situations you're facing. Or asynchronous: an ongoing written conversation on a platform that suits us both. Some of the most useful mentoring fits between meetings, not inside them.
</div>

<div class="leadership-card" markdown="1">
**Deliberately small.** I keep **2 mentoring positions**, and I open them when I can give them proper attention. Depth over throughput. I'd rather do right by two people than badly by ten.
</div>

When a position opens, I'll state the availability here and you can simply email me to take it. No booking software, no scheduling links, just a conversation to check we're a good fit.

## Current availability
{: .icon-explore}

**Both positions are currently unavailable.**

Want to hear when a position opens? [Email me](mailto:vasileiadis.nikos@gmail.com?subject=Mentoring%20—%20keep%20me%20posted) with a couple of lines about who you are and what you'd want from mentoring, and I'll let you know when availability comes up.

## What I ask of you

Bring any situation. Be willing to talk about what's actually hard, not the polished version. And expect the learning to run both ways. I take as much from these conversations as I give.

---

**Not sure if this is for you?** [Ask me](mailto:vasileiadis.nikos@gmail.com). The worst that happens is a good conversation.
