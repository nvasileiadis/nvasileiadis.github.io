...
On an org chart, the plan looked like housekeeping. Restructurings are conducted in the org chart's language — boxes, lines, headcounts — and in that language, merging a four-person team into a neighbouring one reads as pure simplification. One less box. One less manager to coordinate. Nobody proposing it was careless or hostile; the drawing they were working from simply had no way to represent what the box did.

What the chart couldn't show was the web. Twenty-plus teams consumed those components daily. Every customer touchpoint shared one visual language because it was defined in exactly one place. Accessibility and brand standards were enforced once, not re-derived twenty times. Dissolving the team wouldn't dissolve any of that demand — it would just remove the centre. Within a sprint or two, each consuming team would quietly start building its own components; the divergence would compound invisibly until the day the company needed its whole estate to move at once. That day was closer than anyone knew.
...
The first instalment arrived about eighteen months later, when the company rebranded its entire digital estate: 522 pages, on the critical path for every team's launch, shipped in two quarters. That speed was possible for exactly one reason — a single, coherent design system existed, so the rebrand meant updating one source of truth and propagating it, rather than renegotiating the brand twenty times with twenty divergent component sets. Priced against the dissolved-team counterfactual, the same programme reads as years and a small army of contractors.
...
A complaint is a request to be trusted. A diagram is an invitation to check. When something load-bearing is about to be deleted, bring the picture.
